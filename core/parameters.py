...
class ParameterBase(object):
    ...
    def _initiate_attributes(self): 
        ...
        self.area = None        # Dedicated for a core.Area object 
    ...
    def _get_closest_match_in_pos(self, lat=None, lon=None, tolerance_filter=None, boolean=True, df=False): 
        ...
        def calc_dist(x): 
            ...
            if lat == x[0] and lon == x[1]:
                return 0
            return utils.latlon_distance([lat, lon], 
                                             [x[0], x[1]])
...
#        dh = core.DataHandler('matching_data')
...
#        new_df = pd.concat([self.ntra.column_data, df_din], axis=1)
        ...
        self.data_handler = core.DataHandler('calculated_din')
...
if __name__ == '__main__':
    ...
    totn = ParameterTOTN()
    totn.set_data_handler(filtered_data)
    totn.filter_data(winter_filter_1)
    
    df_row = pd.Series(totn.get_df_row_for_index(0))
    
    t = pd.Timestamp(datetime.datetime(2000, 1, 17, 9))
    df_row['time'] = t
    
#    m = totn._get_closest_match_in_time(datetime_object=t, tolerance_filter=tolerance_filter)
    
    value_row = totn.get_closest_matching_data(df_row=df_row, tolerance_filter=tolerance_filter) 
