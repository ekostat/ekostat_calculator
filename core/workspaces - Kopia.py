...
class Subset(object):
    ...
    def _load_subset_config(self): 
        self.config = Config(self.subset_directory + '/subset.cfg')
...
class WorkSpace(object):
    ...
    def _initiate_attributes(self): 
        ...
        self.directory_path_raw_data = self.directory_path_input_data + '/raw_data'
    ...
    def _setup_workspace(self):
        ...
        if not os.path.exists(self.directory_path_raw_data):
            os.makedirs(self.directory_path_raw_data)
...
#            self.cfg[indicator] = self.cf_df.groupby(QE)['parameters'].split(',')     
    ...
    def _load_workspace_config(self): 
        self.config = Config(self.workspace_directory + '/workspace.cfg')   
    ...
    def import_file(self, file_path=None, data_type=None):
        ...
        # Copy file
        target_file_path = '/'.join([directory_path_raw_data, os.path.basename(file_path)])
    ...
    def import_default_data(self):
        ...
        if os.listdir(self.directory_path_raw_data):
            print('raw_data directory is not empty. Will not copy default files from resource directory!')
            return False
        
        source_directory = self.resource_directory + '/default_data'  
        ...
        for file_name in file_name_list: 
            ...
            tar = '/'.join([self.directory_path_raw_data, file_name])
...
#        print('== {}'.format(sub))
    ...
    def _load_all_data(self): 
        ...
        """
        self.dtype_settings = core.RawDataFiles(self.directory_path_raw_data)
        ...
        for file_path, data_type in self.dtype_settings.get_active_paths_with_data_type(): 
            ...
            if data_type == 'phyche':
                self.data_handler.physical_chemical.load_source(file_path=file_path,
                                                                raw_data_copy=True)
                data_loaded = True
