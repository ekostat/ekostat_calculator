...
# ekos.copy_workspace(source_uuid='default_workspace', target_alias='kuszonsmodellen')
...
workspace_uuid = ekos.get_unique_id_for_alias(workspace_alias = 'lena_indicator') #'kuszonsmodellen'
...
print(subset_alias, subset_uuid)
w.set_data_filter(subset = subset_uuid, step=1, 
                         filter_type='include_list', 
                         filter_name='MYEAR', 
                         data=['1985', '2007', '2008', '2009', '2010', '2011', '2012']) 
w.set_data_filter(subset = subset_uuid, step=1, 
                         filter_type='include_list', 
                         filter_name='viss_eu_cd', data = ['SE584340-174401', 'SE581700-113000'])
...
w.get_step_object(step = 3, subset = subset_uuid).calculate_quality_element(subset_unique_id = subset_uuid, quality_element = 'nutrients')
...
w.get_step_object(step = 3, subset = subset_uuid).calculate_quality_element(subset_unique_id = subset_uuid, quality_element = 'bottomfauna')
w.get_step_object(step = 3, subset = subset_uuid).calculate_quality_element(subset_unique_id = subset_uuid, quality_element = 'oxygen')
w.get_step_object(step = 3, subset = subset_uuid).calculate_quality_element(subset_unique_id = subset_uuid, quality_element = 'secchi')
